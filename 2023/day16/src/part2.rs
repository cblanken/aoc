...
impl LightGrid {
    ...
    fn reset(&mut self) {
        self.egrid = vec![vec![0; self.width as usize]; self.height as usize];
    }
...
pub fn solve(filepath: &str) -> String {
    ...
    // Test all possible light beam entry points
    let mut max_energized = 0;
    for x in 0..lgrid.width {
        lgrid.energize(Pos(0, x), DIR::SOUTH, 0);
        max_energized = max_energized.max(lgrid.get_energized_tile_count());
        lgrid.reset();

        lgrid.energize(Pos(lgrid.height-1, x), DIR::NORTH, 0);
        max_energized = max_energized.max(lgrid.get_energized_tile_count());
        lgrid.reset();
    }

    for y in 0..lgrid.width {
        lgrid.energize(Pos(y, 0), DIR::EAST, 0);
        max_energized = max_energized.max(lgrid.get_energized_tile_count());
        lgrid.reset();

        lgrid.energize(Pos(0, lgrid.width-1), DIR::WEST, 0);
        max_energized = max_energized.max(lgrid.get_energized_tile_count());
        lgrid.reset();
    }
